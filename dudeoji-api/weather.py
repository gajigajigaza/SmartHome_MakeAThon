...
jh 수정함 - 기상청 API가 격자 범위 밖 등의 좌표에서 예외 없이 sentinel성
비정상 값(-999 등)을 반환하는 경우가 있어(제주 좌표에서 실측 확인),
fetch_outdoor_weather()가 필드별 정상 범위(_WEATHER_VALUE_RANGES)를 벗어난
값은 결과 딕셔너리에서 제외합니다. 이 경우는 예외가 아니라 "해당 필드만
빠진 정상 응답"으로 처리됩니다.
"""
import asyncio
import math
...
# jh 수정함 - 기상청 API가 격자 범위 밖 등의 좌표에서 예외를 던지지 않고
# sentinel성 비정상 값(-999 등)을 그냥 반환하는 경우가 실측으로 확인됨
# (제주 lat=33.49 좌표에서 outdoor_temperature=-999.0, feels_like가 그 값으로
# 파생 계산되며 천문학적으로 튐). 필드별 정상 범위를 벗어나면 결과에서 제외한다.
_WEATHER_VALUE_RANGES = {
    "outdoor_temperature": (-50.0, 60.0),
    "outdoor_humidity": (0.0, 100.0),
    "wind_speed": (0.0, 100.0),
    "feels_like": (-50.0, 60.0),
}
...
async def fetch_outdoor_weather(latitude: float, longitude: float) -> OutdoorWeather:
    ...
    DB/다른 테이블 조회 없이 받은 위경도로 바로 두 API를 호출한다. 기상청 응답이
    정상 범위를 벗어난 값(_WEATHER_VALUE_RANGES 참고)을 주면 그 필드는 결과
    딕셔너리에서 아예 제외한다 — readings_router.py의 save_reading_for_user()가
    outdoor_temperature/outdoor_humidity에 대해 이미 "딕셔너리에 없으면(None이면)
    센서가 보낸 기존 값을 그대로 쓴다"는 로직을 갖고 있어서, 여기서 빼기만 하면
    자동으로 원래 센서값이 유지된다.
    """
    ...
    combined = {**weather, **air_pollution}

    for field_name, (min_value, max_value) in _WEATHER_VALUE_RANGES.items():
        value = combined.get(field_name)
        if value is None:
            continue
        if value < min_value or value > max_value:
            print(
                f"[weather.py] 비정상 값 감지 - {field_name}={value} "
                f"(lat={latitude}, lon={longitude}, 허용범위 {min_value}~{max_value}) "
                "→ 결과에서 제외"
            )
            del combined[field_name]
