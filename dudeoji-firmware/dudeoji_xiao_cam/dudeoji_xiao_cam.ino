// XIAO ESP32S3 Sense 카메라 보드.
// STA 모드로 공유기에 접속해 http://<MDNS_NAME>.local 에서 두 가지를 제공한다:
//   /       데이터 수집용 웹페이지 (라벨 입력 후 캡처 -> ZIP 다운로드, person/no_person 수집용)
//   /jpg    현재 프레임 1장 (라즈베리파이 추론 서버가 주기적으로 이 엔드포인트를 GET한다)
//
// 이 보드는 촬영만 한다 - 사람 감지 추론은 라즈베리파이에서 돈다.
#include <WiFi.h>
#include <WebServer.h>
#include <ESPmDNS.h>
...
<title>Dudeoji Cam - 데이터 수집기</title>
<style>
...
<h2>두더지 카메라 데이터 수집기</h2>
<img id="cam" src="/jpg">
<div>
<input id="label" placeholder="라벨 (예: person)" value="person">
...
function zipAll(){
  ...
  a.href=URL.createObjectURL(blob); a.download='dudeoji_person_dataset.zip'; a.click();
...
void setup() {
  ...
  // 공유기(STA)에 접속한다 - 실패하면 단독 핫스팟으로 대체
  WiFi.mode(WIFI_STA);
